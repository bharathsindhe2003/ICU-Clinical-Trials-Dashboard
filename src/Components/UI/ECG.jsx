...
export default function ECG({ pdfData, isVisible, headerImageSrc }) {
  const [selectedUuid, setSelectedUuid] = useState(null);
  const [page, setPage] = useState(0);
  const uuids = useMemo(() => Object.keys(pdfData ?? {}), [pdfData]);
  const PAGE_SIZE = 6;
  ...
  const firstVisibleUuid = visibleUuids[0] ?? firstUuid;
  const activeUuid =
    selectedUuid && pdfData?.[selectedUuid] ? selectedUuid : firstVisibleUuid;
      ...
      >
        <Box
          sx={{
            ...
            fontSize: 32,
            px: 1,
            color: safePage > 0 ? "primary.main" : "grey.400",
            userSelect: "none",
          }}
          onClick={() => safePage > 0 && setPage(safePage - 1)}
        >
        ...
        <Box
          sx={{
            cursor: safePage < maxPage ? "pointer" : "not-allowed",
            fontSize: 32,
            px: 1,
            color: safePage < maxPage ? "primary.main" : "grey.400",
            userSelect: "none",
          }}
          onClick={() => safePage < maxPage && setPage(safePage + 1)}
        >
      ...
      <Box sx={{ width: "100%", overscrollBehavior: "contain" }}>
        {activeUuid && pdfData[activeUuid] && (
          <Box sx={{ mt: 2, width: "100%" }}>
            {/* ── centred image above the two PDFs ─────────────────────────── */}
            <CentreImage src={pdfData[activeUuid].ecg_pdfURL} />
          

            {/* ── Svasthya + ICU side by side ───────────────────────────────── */}
            <Box
              sx={{
                display: "flex",
                flexDirection: { xs: "column", md: "row" },
            ...
            >
              <PDFViewport
                key={`svs-${activeUuid}`}
                title="Svasthya"
                url={pdfData[activeUuid].svs_pdfURL}
                type={0}
              />
              <PDFViewport
                key={`icu-${activeUuid}`}
                title="ICU"
                url={pdfData[activeUuid].icu_pdfURL}
                type={3}
              />
            </Box>
          </Box>
