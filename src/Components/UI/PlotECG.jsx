...
export default function PlotECG({ data, isVisible }) {
  ...
  const handleECGchange = (newValue) => {
    if (newValue !== selectedECG) {
      setSelectedECG(newValue);
    }
  };
  ...
  return (
    <Box>
      ...
      <Box sx={{ flex: `0 0 ${100 / ecg_type.length}%`, minWidth: 0 }}>
        {/* key=selectedECG → remounts only when tab type actually changes */}
        <ECG
          key={selectedECG}
          pdfData={data?.[selectedECG]}
          isVisible={true}
        />
      </Box>
    </Box>
